...
setwd("~/Documents/Current_Projects/Thermal Selection Experiment")
...
dataseq = read.csv("ExperimentData/ThermalKnockdownResults_SequencedSamplesOnly.csv", header = T)[-1]
...
# 140 in control (77 F, 74 M), 111 in high-temp treatment (56 F, 55 M)

# sequenced samples only
dataseq %>% group_by(Sex, Treatment) %>% 
  dplyr::summarise(NumSamples = sum(Count)) %>% as.data.frame()
# 127 in control (64 F, 63 M), 107 in high-temp treatment (53 F, 54 M)

# Visualize variation in knockdown time
ggplot(data, aes(x=Treatment, y=KDtime, fill=Treatment)) +
...
ggplot(data, aes(x=Treatment, y=WingLength, fill=Treatment)) +
  geom_boxplot(outlier.shape = NA) + 
...
mod = lmer(KDtime ~ Treatment + Sex + (1|ExpRound) + (1|Trial), data = data)
...
mod2 = lm(WingLength ~ Treatment + Sex, data = data)
summary(mod2)
